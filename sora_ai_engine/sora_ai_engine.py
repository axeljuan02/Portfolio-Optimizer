...
years = int(input(f'{blue}Enter the number of years you want to take into consideration (e.g : 5) : {reset}'))
start_date = end_date - timedelta(years*365)

tickers = input(f'{blue}Enter the assets you own by their tickers and separated by a comma (e.g : AMZN, AAPL, NVDA, GOOGL) : {reset}')
ticker_list = tickers.split(sep=',')
ticker_list = [t.strip() for t in ticker_list]

weights = input(f'{blue}Enter your allocation for each asset separated by a comma (e.g : 0.25,0.25,0.25,0.25) : {reset}')
...
nb_sim = int(input(f'{blue}Enter the number of simulations desired for the Monte Carlo method. (1000 to 100000) :{reset}'))
